...
public class StateMachineEnvironment {
	 ...
	 * 
	 * returns the state number if the state was reset, otherwise returns negative number
	 */

    Random randoSquew = new Random(System.currentTimeMillis());
	public int reset() {
		if (!reset){ //only return if the flag is set
			return -1;
		}
	...
	public Sensors tick(char move) {
		...
		// A Sensors object to keep track of the agent's
		// sensors. The first represents if  is in a new
		...
		//update all sensor values for new sensor object based on new state
		newSensors.EVEN_SENSOR = false;
		if (newState % 2 == 0){
			newSensors.EVEN_SENSOR = true;
		}
		
		newSensors.STATE_NUMBER = newState;
		//future sensors should be updated here
	
		// If we have reached the goal, update the goal sensor
		if(newState == GOAL_STATE){
			newSensors.GOAL_SENSOR = true;
			reset = true;
		}
