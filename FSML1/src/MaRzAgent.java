...
public class MaRzAgent extends Agent {
	...
	public class SuffixNode {
		...
		// public int queSeqIndex; //possible index for each node; NOTE: Might
		// speed things up but not likely
		public double heristic;
		...
		public SuffixNode(String initSuffix, int initFailCount, int initTries,
				Double initHeristic, boolean initInitialNode) {
			this.suffix = initSuffix;
			this.failedTries = initFailCount;
			this.tries = initTries;
			this.queueSeq = "";
			this.heristic = initHeristic;
			this.initialNode = initInitialNode;
		}// ctor
	...
	public MaRzAgent() {
		...
		SuffixNode initNode = new SuffixNode("", 0, 0, 0.0, true);
	...
	public void splitNode(String aSuffix){
		...
		for(int i=0; i < alphabet.length; i++){
			...
			System.arraycopy(updateTries(alphabet[i] + aSuffix), 0, tempTries, 0, 2);
			double heristic = 0.0;
			if(tempTries[0] == 0){
				heristic = (activeNode.g * G_WEIGHT);
			}else{
				heristic = (tempTries[1] / tempTries[0]) + (activeNode.g * G_WEIGHT);
			}
			debugPrintln("SPLITTING NODE INTO: " + (alphabet[i] + aSuffix) + " fails: " + tempTries[1] + " total tries: " + tempTries[0]);
			SuffixNode aNode = new SuffixNode(alphabet[i] + aSuffix, tempTries[1], tempTries[0], heristic, false);
	 ...
	 * 
	 * 9/7/16 http://stackoverflow.com/questions/11420920/search-a-hashmap-in-an-arraylist-of-hashmap
	 * 
	 */
	...
	public SuffixNode findBestNodeToTry(){
		...
		for(Map.Entry<SuffixNode,String> entry : hashFringe.entrySet()){
			if(theBEASLIESTCombo > entry.getKey().heristic ){
				theBEASLIESTCombo = entry.getKey().heristic;
	...
	public void trySeq() {
		...
		if(activeNode.tries != 0){
			activeNode.heristic = (activeNode.failedTries / activeNode.tries)
				+ (activeNode.g * G_WEIGHT);
		}
		else{
			activeNode.heristic = (activeNode.g * G_WEIGHT);
		}
	 ...
	 * @return
	 */
	public ArrayList<Integer> getIndexOfSuffix(String aString, String suffix) {
		String word = aString;
	...
	public static void tryGenLearningCurves() {
		...
		try {
			...
			for (int i = 1; i <= NUM_MACHINES; ++i) {
				...
				System.out.println("Average Solution Length (Cheating): " + gilligan.env.avgStepsToGoalWithPath(gilligan.env.shortestBlindPathToGoal()));
