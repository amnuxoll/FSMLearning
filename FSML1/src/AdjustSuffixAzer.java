 ...
 *
 */
public class AdjustSuffixAzer extends Agent
{
...
//                } while (prefixNode != this);
    ...
    public void splitNode()
    {
        ...
        if ( fail || success) {
            ...
            proceed = false; //AZER IS OFF for adjust suffix value attempt
        ...
        do {
            ...
            else // possible success
            {
                ...
                else  //found the goal before sequence finished
                {
                    ...
                    /*
                    if (!queuedLastSeq.equals("")) {
                        Iterator it = prefixRoot.suffixHash.entrySet().iterator();
                        //regex to find matches among prefix frontier nodes in hashfringe
                        while (it.hasNext()) {
                            Map.Entry pair = (Map.Entry)it.next();
                            SuffixNode temp = prefixRoot.suffixHash.get(pair.getKey());
                            if(nextSeqToTry.contains(temp.suffix))
                            {
                                int index = this.memory.length() - temp.suffix.length();
                                temp.successIndexList.add(index);
                            }
        ...
        try
        {
            ...
            for (int i = 1; i <= NUM_MACHINES; ++i)
            {
                ...
                System.out.println("Starting on Machine " + i + " of "
                        + NUM_MACHINES);
                AdjustSuffixAzer gilligan = new AdjustSuffixAzer();
                ...
                System.out.println("The amount of Marz Sequences was: " + gilligan.marzCount);
                System.out.println("The amount of Adjust Sequences was: " + gilligan.adjustCount);
