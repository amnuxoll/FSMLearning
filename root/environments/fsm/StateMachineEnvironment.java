...
public class StateMachineEnvironment  {
    ...
    public void generateStateMachine(){
        ...
        transition = new int[NUM_STATES][this.moves.size()];
        TRANSITIONS_DONE = 0;
    ...
    public void printStateMachine() {
        System.out.print("     ");
        for(int i = 0; i < ALPHABET_SIZE; ++i) {
            System.out.printf("%3c", this.moves.get(i));
        }
        System.out.println();
        ...
        for (int i = 0; i < NUM_STATES - 1; i++) {
            ...
            for (int j = 0; j < NUM_STATES; j++) {
                ...
                for (int k = 0; k < this.moves.size(); k++) {
                    if (transition[i][k] == j)
                    {
                        actions = actions + this.moves.get(k) + ",";
                    }
                }
	...
	public SensorData tick(Move move) {
		...
		int newState = transition[currentState][this.moves.indexOf(move)];
		...
		if(newState != currentState){
		    if(loops.contains(newState)){
                ...
                sensorData.setSensor("IsLoop", true);
            }
            else{
		        loops.add(newState);
            }
			currentState = newState;
		    sensorData.setSensor("NewState", true);
		}

		sensorData.setSensor("Even", newState % 2 == 0);

        if(limitedStateNumbers.contains(newState))
        {
            sensorData.setSensor("LimitedStateNum", newState);
        }

        sensorData.setSensor("StateNum", newState);
        sensorData.setSensor("ModBudgets", newState%1);
        sensorData.setSensor("Distance", paths[currentState].length());

		if(random.nextInt(1) == 1)
            sensorData.setSensor("Noise", true);
	...
	private void findShortestPaths() {
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < NUM_STATES; i++) {
				...
				//onto the queue.
				if (transitionChar != -1) {
					paths[i] = this.moves.get(transitionChar) + paths[currState];
					queue.add(i);
				}
			}
	...
	public int pathResult(int begin, String path) {
        ...
        for(int i = 0; i < path.length(); ++i) {
            ...
            currState = transition[currState][this.moves.indexOf(action)];
    ...
    public int calcDestinationStates(String path, int[] currStates) {
        ...
        for(int c = 0; c < path.length(); ++c) {
            ...
            for(int i = 0; i < NUM_STATES; ++i) {
                if (currStates[i] != GOAL_STATE) {
                    currStates[i] = transition[currStates[i]][this.moves.indexOf(action)];
                    if (currStates[i] == GOAL_STATE) {
                        sum += c + 1;
                        goalCount++;
                    }
